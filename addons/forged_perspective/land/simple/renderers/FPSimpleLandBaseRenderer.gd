class_name FPSimpleLandBaseRenderer
extends Node2D
...
func _enter_tree():
	...
	assert(_fpLandController.get("__is_FPLandController"),
		"FPSimpleLandBaseRenderer: the immediate of this must be an 'FPLandController'")
...
func _draw():
	assert(_fpLandController,
		"FPSimpleLandBaseRenderer: in '_draw()', '_fpLandController' was null! This shouldn't happen!")
	for part in _fpLandController.landParts:
		var baseSuccess := _baseDrawPart(part)
		if not baseSuccess:
			assert(false,
				"FPSimpleLandBaseRenderer: in '_draw()', unknown part type encountered. You may need a custom renderer to extend this.")

func _baseDrawPart(part) -> bool:
	assert(false, "FPSimpleLandBaseRenderer: '_baseDrawPart(part)' must be implemented")
	return false
